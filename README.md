...
这是一个面向电商上架场景的 AI Agent 案例。它从 B2B 货源链接中提取真实商品事实, 过滤批发供货语境, 生成可用于 C 端商品发布的标题、属性、主图、详情页和详情长图。
...
仓库保留展示所需的生成结果、静态 demo、案例输入输出摘要和工程说明文档。账号凭据、原始页面快照和非必要商家信息不作为公开内容发布。
